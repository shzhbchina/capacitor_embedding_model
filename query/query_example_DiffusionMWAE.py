...
import joblib,os
from dataset_embedding.models.DiffusionMixedWassersteinAutoEncoder import DiffusionMixedWAE
import torch
import pandas as pd
import numpy as np
...
model=DiffusionMixedWAE(input_dim=dataset.shape[1], hidden_dim=32*2, latent_dim=8,disc_dim=4,discrete_feature_list=discrete_feature_list)
model_name='DiffusionMWAE'
...
[[pred_x, x_noisy, logits_vectors], z, noise, pred_noise]=model(test_input_dataset)
test_output_dataset=test_query_datasheet.find_component_from_latent_space(z,mixed_transform=True)
test_input_dataset_original=dataset.data_drop_fillna.iloc[0:1000]
#test=(test_output_dataset.iloc[:,0:2]-test_input_dataset_original.iloc[:,0:2])/test_output_dataset.iloc[:,0:2]
