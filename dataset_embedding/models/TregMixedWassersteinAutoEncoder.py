...
class AutoregressiveTabDecoder(nn.Module):
    def __init__(self, latent_dim, hidden_dim, out_dim, n_heads=4, n_layers=2):
        super().__init__()
        self.out_dim = out_dim
        # 可学习的目标token初始embedding（每个字段一个token）
        self.target_token_embed = nn.Parameter(torch.zeros(1, out_dim, hidden_dim))
        nn.init.xavier_uniform_(self.target_token_embed)
        # z条件信息（全局向量）投影为hidden_dim
        self.z_proj = nn.Linear(latent_dim, hidden_dim)
        # Transformer Decoder层
        decoder_layer = nn.TransformerDecoderLayer(d_model=hidden_dim, nhead=n_heads, dim_feedforward=hidden_dim*2, batch_first=True)
        self.decoder = nn.TransformerDecoder(decoder_layer, num_layers=n_layers)
        self.out_proj = nn.Linear(hidden_dim, 1)

    def forward(self, z, teacher_values=None):
        '''
        z: (B, latent_dim)
        teacher_values: (B, out_dim)  # 训练时用，teacher-forcing
        '''
        B = z.size(0)
        # 目标token（待生成各列）初始embedding，expand为(B, out_dim, H)
        tgt = self.target_token_embed.expand(B, self.out_dim, -1)  # (B, out_dim, H)
        # 条件z投影加到每个token
        z_cond = self.z_proj(z).unsqueeze(1)  # (B, 1, H)
        tgt = tgt + z_cond
        # 做未来位mask，实现自回归
        tgt_mask = torch.triu(torch.ones(self.out_dim, self.out_dim, device=z.device), diagonal=1).bool()
        # 解码
        out = self.decoder(tgt, memory=tgt, tgt_mask=tgt_mask)
        out = self.out_proj(out).squeeze(-1)  # (B, out_dim)
        return out


# ------- MixedWAE主模型（接口完全不变，直接替换） -------
class TregMWAE(nn.Module):
    def __init__(self, input_dim, hidden_dim, latent_dim, disc_dim, discrete_feature_list):
        super(TregMWAE, self).__init__()
        ...
        # 解码器
        self.decoder = AutoregressiveTabDecoder(
            latent_dim=latent_dim,
            hidden_dim=hidden_dim,
