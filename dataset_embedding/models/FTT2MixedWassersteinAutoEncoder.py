...
class FTT2MixedWAE(nn.Module):
    def __init__(self, input_dim, hidden_dim, latent_dim, discrete_feature_list,
        ...
        self.decoder_feature_tokenizer = FeatureTokenizer(
            num_continuous=latent_dim,
        ...
        self.decoder_cls_token = nn.Parameter(torch.randn(1, 1, embed_dim))
    ...
    def forward(self, x):
        ...
        decoder_tokens = self.decoder_feature_tokenizer(z, None)
        decoder_cls_tokens = self.decoder_cls_token.expand(z.shape[0], -1, -1)
        decoder_tokens_with_cls = torch.cat([decoder_cls_tokens, decoder_tokens], dim=1)  # Shape: [B, latent_dim + 1, D]
        decoded_tokens = self.transformer_decoder(decoder_tokens_with_cls)
        decoder_cls_output = decoded_tokens[:, 0]  # 取出第一個 token ([CLS]) [B,D]

        # --- Decoding ---
        # 使用潛在向量 z 來重建原始數據
        hidden_rep = self.decoder_mlp(decoder_cls_output)
    ...
    def decode(self, z):
        with torch.no_grad():
            decoder_tokens = self.decoder_feature_tokenizer(z, None)
            decoder_cls_tokens = self.decoder_cls_token.expand(z.shape[0], -1, -1)
            decoder_tokens_with_cls = torch.cat([decoder_cls_tokens, decoder_tokens],
                                                dim=1)  # Shape: [B, latent_dim + 1, D]
            decoded_tokens = self.transformer_decoder(decoder_tokens_with_cls)
            decoder_cls_output = decoded_tokens[:, 0]  # 取出第一個 token ([CLS]) [B,D]
