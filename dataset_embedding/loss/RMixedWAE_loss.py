...
def RMixedWAE_loss( recon_x, x, z,true_table,disc_table,loss_conf=None):
    ...
    # loss_cat = 0
    # for i, logits in enumerate(recon_disc_logits_x):
    #     loss_cat += F.cross_entropy(logits, data_disc_x[:, i].long())*true_table[disc_table][i]
    # recon_loss = loss_cont + loss_cat

    #z = torch.randn_like(z) #test mmd z
    ...
    # --------- 弱正则：z 的中心 / 方差 ----------
    z_mean = z.mean(dim=0)                     # [Dz]
    z_var  = z.var(dim=0, unbiased=False)      # [Dz]
