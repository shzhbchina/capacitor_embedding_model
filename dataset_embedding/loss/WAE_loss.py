...
def WAE_loss( recon_x, x, z,true_table):
    ...
    true_table[:,0]=3*true_table[:,0]
    true_table[:, 0] = 3 * true_table[:, 0]

    recon_loss = reconstruction_loss_scale * F.mse_loss(
        (recon_x*true_table).reshape(x.shape[0], -1), (x*true_table).reshape(x.shape[0], -1), reduction="none"
    ).sum(dim=-1)
